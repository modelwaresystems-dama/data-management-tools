...
(function(){
  ...
  function contextSvg(ms,mid){
    ...
    if(R.length){ const sx=cR-34, top=R[0][2], bot=R[R.length-1][2]+bh(R[R.length-1][1].length); const cmid=C.length?Math.round((C[0][1]+C[C.length-1][1]+48)/2):Math.round((top+bot)/2);
      s.push('<line x1="'+sx+'" y1="'+Math.min(top,cmid)+'" x2="'+sx+'" y2="'+Math.max(bot,cmid)+'" stroke="#6a3d9a" stroke-width="1.6"/><path d="M'+(cC+bw)+','+cmid+' L'+sx+','+cmid+'" fill="none" stroke="#6a3d9a" stroke-width="1.6"/>'); }
    ...
    ["Left arrows: one per managed element and Global region pair, from the element whose states the contribution requires to the region it gates (dashed: no required state recorded).",
     ...
     "which managed element a coupling belongs to). out: declared by this KA (* event emitted) · in: declared by the other KA against this one."].forEach((ln,i)=>s.push('<text x="'+cL+'" y="'+(Hh-36+i*13)+'" fill="#5f6f78" font-size="10.5">'+e(ln)+'</text>'));
  ...
  function chapter(ms,mid){
    ...
    h.push('<hr class="docpb"><h2 class="doch2" id="doc-reference">Reference</h2><h3 class="doch3" id="doc-regions">'+(isG?'The four regions of the Data Asset':'The FTSs: one region per managed element')+'</h3><p class="lede">Each region is one Finite State Transition model: its question, its states, and its own transitions with their events, guards, decision rights, services and cross-region constraints. A transition never crosses regions.</p>');
