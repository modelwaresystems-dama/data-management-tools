...
STATES = [
    ...
    ("STS-PRD-06", "REG-DWBI-PRD", "Product Monitoring Alert", False, False, "BI activity monitoring flags a usage, satisfaction, performance or data issue; the product stays available while investigated.", "The alert, its owner and the DG issue reference remain explicit.", ["deliverable:BI Activity Monitoring"]),
...
EVENTS = {
    ...
    "EV-PRD-01": ("Product request", "Request"), "EV-PRD-02": ("Build completion", "Evidence trigger"), "EV-PRD-03": ("Product release", "Decision outcome"), "EV-PRD-04": ("Build rejection", "Decision outcome"), "EV-PRD-05": ("Adoption confirmation", "Assessment outcome"), "EV-PRD-06": ("Monitoring alert", "Monitoring trigger"), "EV-PRD-07": ("Alert resolution", "Assessment outcome"), "EV-PRD-08": ("Product retirement", "Decision outcome"), "EV-PRD-09": ("Product change request", "Request"),
...
TRANS = [
    ...
    ("TR-PRD-06", "Raise Product Alert", "STS-PRD-05", "STS-PRD-06", "EV-PRD-06", "BI activity monitoring flags a usage, satisfaction, performance or data issue; a Data Asset issue is logged and the assurance trigger emitted.", "DR-DWBI-07", ["SVC-DWBI-08"], ["ACT-DWBI-6"]),
    ("TR-PRD-07", "Resolve Product Alert", "STS-PRD-06", "STS-PRD-05", "EV-PRD-07", "The cause is resolved and the metrics recover.", "DR-DWBI-07", ["SVC-DWBI-08"], ["ACT-DWBI-6"]),
    ...
    ("TR-PRD-10", "Retire Product on Alert", "STS-PRD-06", "STS-PRD-07", "EV-PRD-08", "An unresolved alert leads to withdrawal under the release plan.", "DR-DWBI-06", ["SVC-DWBI-07"], ["ACT-DWBI-6"]),
...
ACTS = [
    ...
    ("ACT-DWBI-6", "Maintain Data Products", "6", "Transition-causing", ["REG-DWBI-DWH", "REG-DWBI-POP", "REG-DWBI-PRD", "REG-DWBI-BIP"], ["TR-DWH-03", "TR-DWH-05", "TR-DWH-06", "TR-DWH-07", "TR-DWH-08", "TR-POP-09", "TR-POP-10", "TR-PRD-05", "TR-PRD-06", "TR-PRD-07", "TR-PRD-09", "TR-PRD-10", "TR-PRD-11", "TR-BIP-04", "TR-BIP-05", "TR-BIP-07"], ["SVC-DWBI-03", "SVC-DWBI-08"]),
]
ARTEFACTS = [
...
CONTRIB = [
    ...
    ("CON-DWBI-02", "TR-AV-01", "guard", {"PRD": ["STS-PRD-04", "STS-PRD-05", "STS-PRD-06"], "POP": ["STS-POP-04", "STS-POP-06"]}, "Access to a data product is released only when the product is released under the release plan on a reconciled population with lineage in the dictionary.", "(DWBI_product_released and DWBI_population_reconciled) or not DWBI_product_active", "Conditional", "Applies to the data product as its own asset instance."),
    ("CON-DWBI-03", "TR-AV-04", "guard", {"PRD": ["STS-PRD-04", "STS-PRD-05"]}, "Suspended access to a data product is restored only when the product is released and free of alerts.", "DWBI_product_released_clear or not DWBI_product_active", "Conditional", ""),
    ...
    ("CON-DWBI-05", "TR-AS-05", "event", {"POP": ["STS-POP-05"], "DWH": ["STS-DWH-05"], "PRD": ["STS-PRD-06"]}, "A failed load, a warehouse performance breach or a product monitoring alert is a material change affecting the assurance claim.", "DWBI_load_failed or DWBI_warehouse_tuning or DWBI_product_alert", "Conditional", "DWBI emits EV-AS-05 when TR-POP-04, TR-POP-08, TR-DWH-05 or TR-PRD-06 fires."),
    ...
    ("CON-DWBI-07", "TR-AV-03", "event", {"PRD": ["STS-PRD-06"]}, "A product monitoring alert on a data or performance issue suspends access to the product until resolved.", "DWBI_product_alert", "Conditional", "DWBI emits EV-AV-03 when TR-PRD-06 fires on a data issue."),
]
KA_COUPLINGS = [
...
FACT_BINDINGS = {
    ...
    "DWBI_product_active": {"region": "REG-DWBI-PRD", "states": ["STS-PRD-02", "STS-PRD-03", "STS-PRD-04", "STS-PRD-05", "STS-PRD-06"]},
    "DWBI_product_released": {"region": "REG-DWBI-PRD", "states": ["STS-PRD-04", "STS-PRD-05", "STS-PRD-06"]},
    "DWBI_product_released_clear": {"region": "REG-DWBI-PRD", "states": ["STS-PRD-04", "STS-PRD-05"]},
    "DWBI_product_alert": {"region": "REG-DWBI-PRD", "states": ["STS-PRD-06"]},
...
XRG = [
    ...
    ("XRG-DWBI-05", "A data product is released or retired only under a release plan in force.", ["TR-PRD-04", "TR-PRD-09", "TR-PRD-10", "TR-PRD-11"], "Required", "BIP in ('STS-BIP-04','STS-BIP-05')"),
