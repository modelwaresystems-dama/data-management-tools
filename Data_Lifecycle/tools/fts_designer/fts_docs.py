...
fts_docs.py  -  generate the FTS documentation set from the models, and read Word review comments back.
  ...
  python fts_docs.py <models_dir> --out <docs_dir> [--scenarios <dir>] [--runs <dir>] [--figures <dir>] [--docx] [--html]
      writes one Word document per model plus the system book (and one single-file HTML book) from the same content.
...
def context_svg(ms, mid):
    ...
    W = 1180; colL, colC, colR = 40, 430, 800; bw = 300; lh = 14
    def box_h(lines): return 34 + lh * max(1, len(lines))
    y = 60; L = []
    for r in gregs:
        lines = by_greg.get(r["id"], []); L.append((r, lines, y)); y += box_h(lines) + 14
    hL = y
    y = 60; C = []
    for r in regs:
        C.append((r, y)); y += 58
    ...
    for r, lines, yy in L:
        ...
        else: s.append(f'<line x1="{colC}" y1="{yy+h/2:.0f}" x2="{colL+bw}" y2="{yy+h/2:.0f}" stroke="#2f6f9a" stroke-width="1.4" marker-end="url(#a)"/>')
    for r, yy in C:
        s.append(f'<rect x="{colC}" y="{yy}" width="{bw}" height="48" rx="6" fill="#e3f1f0" stroke="#0f6b6e"/>')
        s.append(f'<text x="{colC+10}" y="{yy+19}" font-weight="600" fill="#1e2a30">{H.escape(r["name"][:44])}</text>')
        s.append(f'<text x="{colC+10}" y="{yy+36}" fill="#5f6f78" font-size="11">{r["id"]} · {H.escape((r.get("instanceScope") or "")[:40])}</text>')
    ...
    s.append(f'<text x="{colL}" y="{Hh-10}" fill="#5f6f78" font-size="11">G guard · E event · S service · D decision right · * Non-waivable (left) or event emitted into the other KA (right) · out: declared by this KA · in: declared by the other KA against this one</text>')
    s.append("</svg>")
    return "\n".join(s)
...
def build_chapter(ms, mid, figures_dir=None):
    ...
    d.t(["ID", "Transition", "From → to", "Event", "Guard", "Decision right", "Services", "XRG"], rows, "TBL-TRANSITIONS")
    if not is_global:
    ...
    if figures_dir:
        for suffix, cap in [("_simplified.png", "Simplified FTS (Mermaid export)")]:
            p = os.path.join(figures_dir, m["_file"].replace(".fts.json", suffix))
            if os.path.exists(p): d.fig(p, cap, "FIG-SIMPLIFIED")
    d.h(2, "Sources")
    d.ul([f"{s.get('id', '')}: {s.get('source', '')} ({s.get('type', '')}). {s.get('limitations', '')}" for s in m.get("sources", [])] or [m["meta"].get("source", "")])
    return d

# ---------------------------------------------------------------- renderers
def render_html(docs, out_path):
    css = """body{font-family:Segoe UI,Arial,sans-serif;font-size:14px;color:#1e2a30;background:#f6f7f8;margin:0;padding:0 16px 40px}
    nav{position:sticky;top:0;background:#fff;border-bottom:1px solid #d3dbe0;padding:8px 12px;font-size:13px;z-index:2} nav a{margin-right:10px;color:#0f6b6e;text-decoration:none}
    .doc{background:#fff;border:1px solid #d3dbe0;border-radius:6px;padding:20px 24px;margin:20px auto;max-width:1180px}
    h1{font-size:1.5rem;margin:18px 0 6px} h2{font-size:1.15rem;margin:22px 0 8px;color:#0f6b6e} h3{font-size:1rem;margin:16px 0 6px} .sub{color:#5f6f78;font-size:12.5px}
    table{border-collapse:collapse;width:100%;font-size:12px;margin:6px 0 12px} th,td{border:1px solid #d3dbe0;padding:4px 6px;vertical-align:top;text-align:left} th{background:#e6eef5}
    .tw{overflow-x:auto} .id{font-family:Consolas,Menlo,monospace;font-size:11.5px;color:#5f6f78} figure{margin:10px 0} figcaption{font-size:12px;color:#5f6f78} svg{max-width:100%;height:auto} .pb{border-top:2px dashed #d3dbe0;margin:24px 0}"""
    out = [f"<!doctype html><html><head><meta charset='utf-8'><title>FTS documentation set</title><style>{css}</style></head><body>"]
    out.append("<nav>" + " ".join(f"<a href='#doc{i}'>{H.escape(d.title[:38])}</a>" for i, d in enumerate(docs)) + "</nav>")
    for i, d in enumerate(docs):
        out.append(f"<div class='doc' id='doc{i}'><h1>{H.escape(d.title)}</h1><div class='sub'>{H.escape(d.subtitle)}</div>")
        for b in d.blocks:
            if b[0] == "h": out.append(f"<h{b[1]}{' id=' + chr(34) + b[3] + chr(34) if b[3] else ''}>{H.escape(b[2])}</h{b[1]}>")
            elif b[0] == "p": out.append(f"<p>{H.escape(b[1])}</p>")
            elif b[0] == "ul": out.append("<ul>" + "".join(f"<li>{H.escape(x)}</li>" for x in b[1]) + "</ul>")
            elif b[0] == "pb": out.append("<div class='pb'></div>")
            elif b[0] == "fig":
                svg = b[1]
                if svg.lstrip().startswith("<svg"): out.append(f"<figure>{svg}<figcaption>{H.escape(b[2])}</figcaption></figure>")
                elif os.path.exists(svg):
                    import base64; out.append(f"<figure><img src='data:image/png;base64,{base64.b64encode(open(svg, 'rb').read()).decode()}' style='max-width:100%'><figcaption>{H.escape(b[2])}</figcaption></figure>")
            elif b[0] == "t":
                out.append("<div class='tw'><table><thead><tr>" + "".join(f"<th>{H.escape(h)}</th>" for h in b[1]) + "</tr></thead><tbody>")
        ...
        out.append("</div>")
    out.append(f"<div class='sub' style='max-width:1180px;margin:0 auto'>Generated {stamp()} by fts_docs.py v{VERSION} from the models; every ID is the model's ID.</div></body></html>")
    open(out_path, "w", encoding="utf-8").write("\n".join(out)); return out_path
        ...
        if b[0] == "h":
            ...
            if src.lstrip().startswith("<svg"):
                ...
                if key not in png_cache:
                    try:
                        import cairosvg; png_cache[key] = cairosvg.svg2png(bytestring=src.encode("utf-8"), output_width=1800)
                    except Exception: png_cache[key] = None
                png = png_cache[key]
...
def main(argv=None):
    import argparse
    ap = argparse.ArgumentParser(); ap.add_argument("models", nargs="?"); ap.add_argument("--out", default="docs"); ap.add_argument("--scenarios"); ap.add_argument("--runs"); ap.add_argument("--figures")
    ...
    for mid in order: docs.append(build_chapter(ms, mid, a.figures))
    ...
    if a.html or not a.docx:
        written.append(render_html(docs, os.path.join(a.out, "fts_documentation_set.html")))
