#!/usr/bin/env python3
"""
dso_spec.py  -  Data Storage and Operations FTS v0.1: four state regions, each its own FTS over one managed element
...
SRC_HOWARD = "Howard Diesel, 21 Sep 2026: four Data Storage and Operations managed elements (Database Technology; Database Environment; Stored Instance of a Data Asset; Business Continuity Plan), Global gating of materialise, custody, preservation and destruction with the transfer and assurance triggers"
...
KA_SUBJECT = "Data Storage and Operations Knowledge Area: four managed elements, one FTS (state region) each; the Data Asset is the Global subject reached through contributions"
REGIONS = [
    ...
    ("REG-DSO-STO", "Stored Instance of a Data Asset", "STO", "Whether the stored instance of one Data Asset is loaded, protected by a verified backup under the OLA, archived, in migration, replication or versioning, or purged.", "STS-STO-01", "Exactly one active state per asset; Protected means the last backup is verified within the OLA recovery point.",
...
STATES = [
    ...
    ("STS-STO-05", "REG-DSO-STO", "Instance in Migration", False, False, "The instance is being migrated, replicated or versioned to another environment or version under an approved migration plan; the source stays protected until verified.", "Source and target remain reconciled; the migration plan and cut-over remain recorded.", ["process:2.6", "deliverable:Migrated/Replicated/Versioned Data"]),
...
EVENTS = {
    ...
    "EV-STO-01": ("Load request", "Request"), "EV-STO-02": ("Backup verification", "Evidence trigger"), "EV-STO-03": ("Archive decision", "Decision outcome"), "EV-STO-04": ("Restore request", "Request"), "EV-STO-05": ("Migration approval", "Decision outcome"), "EV-STO-06": ("Migration verification", "Evidence trigger"), "EV-STO-07": ("Migration rollback", "Decision outcome"), "EV-STO-08": ("Purge confirmation", "Evidence trigger"), "EV-STO-09": ("Backup failure", "Monitoring trigger"),
...
ACTS = [
    ...
    ("ACT-DSO-2.5", "Manage Test Datasets", "2.5", "Supporting", ["REG-DSO-ENV"], [], ["SVC-DSO-02"]),
    ("ACT-DSO-2.6", "Manage Data Migration", "2.6", "Transition-causing", ["REG-DSO-ENV", "REG-DSO-STO"], ["TR-ENV-07", "TR-ENV-08", "TR-STO-04", "TR-STO-05", "TR-STO-06", "TR-STO-07", "TR-STO-08", "TR-STO-09", "TR-STO-10"], ["SVC-DSO-04", "SVC-DSO-05", "SVC-DSO-06"]),
]
ARTEFACTS = [
    ("ART-DSO-01", "Database Technology Evaluation Criteria", "STS-TEC-03", "Evaluate Database Technology", "Evidences Evaluated Technology."), ("ART-DSO-02", "Database Environments", "STS-ENV-04", "Develop Database Instances", "Evidences Environment in Service; cited by Global materialisation and custody."), ("ART-DSO-03", "Migrated/Replicated/Versioned Data", "STS-STO-05", "Manage Data Migration", "Evidences Instance in Migration; cited by the Global custody transfer trigger."), ("ART-DSO-04", "Business Continuity Plans", "STS-BCP-03", "Plan for Business Continuity", "Evidences Tested Continuity; cited by Global active custody."), ("ART-DSO-05", "Database Performance OLA", "STS-ENV-04", "Manage Database Performance", "Evidences the OLA the environment in service is measured against."),
]
CONTRIB = [
    ("CON-DSO-01", "TR-EX-02", "guard", {"ENV": ["STS-ENV-04"], "STO": ["STS-STO-02", "STS-STO-03", "STS-STO-04", "STS-STO-05"]}, "An asset is materialised only when its instance is loaded in an environment in service.", "DSO_environment_in_service and DSO_instance_loaded", "Required", "TR-STO-01 precedes TR-EX-02 in every scenario."),
    ...
    ("CON-DSO-05", "TR-CP-05", "event", {"STO": ["STS-STO-05"]}, "A migration or replication of the instance to another environment is a custody transfer trigger.", "DSO_instance_migrating", "Conditional", "DSO emits EV-CP-05 when TR-STO-06 targets another custodian's environment."),
...
FACT_BINDINGS = {
    ...
    "DSO_instance_loaded": {"region": "REG-DSO-STO", "states": ["STS-STO-02", "STS-STO-03", "STS-STO-04", "STS-STO-05"]},
...
VECTORS = [
    ("CFG-DSO-01", "Greenfield", {"REG-DSO-TEC": "STS-TEC-01", "REG-DSO-ENV": "STS-ENV-01", "REG-DSO-STO": "STS-STO-01", "REG-DSO-BCP": "STS-BCP-01"}, "Initial configuration: no technology, environment, instance or plan; Global custody and materialisation blocked."),
    ("CFG-DSO-02", "Environment in service, plan tested", {"REG-DSO-TEC": "STS-TEC-05", "REG-DSO-ENV": "STS-ENV-04", "REG-DSO-STO": "STS-STO-01", "REG-DSO-BCP": "STS-BCP-03"}, "Legal: active custody can be established; materialisation waits for the load."),
    ("CFG-DSO-03", "Protected instance", {"REG-DSO-TEC": "STS-TEC-05", "REG-DSO-ENV": "STS-ENV-04", "REG-DSO-STO": "STS-STO-03", "REG-DSO-BCP": "STS-BCP-03"}, "Legal: all DSO custody and materialisation guards satisfied; monitoring supplies assurance evidence."),
    ("CFG-DSO-04", "Archived instance", {"REG-DSO-TEC": "STS-TEC-05", "REG-DSO-ENV": "STS-ENV-04", "REG-DSO-STO": "STS-STO-04", "REG-DSO-BCP": "STS-BCP-03"}, "Legal: preservation custody allowed; restoration waits for the restore."),
    ("CFG-DSO-05", "Degraded environment, plan invoked", {"REG-DSO-TEC": "STS-TEC-05", "REG-DSO-ENV": "STS-ENV-05", "REG-DSO-STO": "STS-STO-03", "REG-DSO-BCP": "STS-BCP-04"}, "Legal: EV-AS-05 emitted; DG issue logged; new materialisation and custody blocked until service is restored."),
    ("CFG-DSO-06", "Purged instance", {"REG-DSO-TEC": "STS-TEC-05", "REG-DSO-ENV": "STS-ENV-04", "REG-DSO-STO": "STS-STO-06", "REG-DSO-BCP": "STS-BCP-03"}, "Legal: destruction and custody closure allowed."),
]
EVIDENCE = [
    ("EVD-DSO-01", "Technology evaluation record", "Decision evidence", "TR-TEC-03", "Evaluation against the criteria; adoption decision and conditions."), ("EVD-DSO-02", "Environment development record", "Control evidence", "TR-ENV-02", "Change scripts, implementation path, naming standards applied."), ("EVD-DSO-03", "Service acceptance record", "Decision evidence", "TR-ENV-03", "Acceptance test against the OLA; monitoring activation."), ("EVD-DSO-04", "Monitoring report", "Assurance evidence", "TR-ENV-05", "Performance, storage, operations and service metrics; breach findings."), ("EVD-DSO-05", "Load record", "Control evidence", "TR-STO-01", "Source, environment, model version, row counts."), ("EVD-DSO-06", "Backup verification", "Control evidence", "TR-STO-02", "Backup, verification result, recovery point."), ("EVD-DSO-07", "Archive record", "Control evidence", "TR-STO-04", "Archive copy, verification, retention."), ("EVD-DSO-08", "Migration reconciliation", "Control evidence", "TR-STO-07", "Source and target reconciliation; cut-over."), ("EVD-DSO-09", "Purge record", "Control evidence", "TR-STO-09", "Scope, method, confirmation of instance and backups."), ("EVD-DSO-10", "Continuity test record", "Risk evidence", "TR-BCP-02", "Test scenario, recovery objectives, result."), ("EVD-DSO-11", "Invocation and recovery record", "Risk evidence", "TR-BCP-05", "Continuity event, invocation, recovery against objectives."),
]
...
SPEC = {
    "meta": {"modelId": "KA-DSO", "name": "Data Storage and Operations FTS", "knowledgeArea": "Data Storage and Operations", "version": "0.1", "subjectType": KA_SUBJECT,
             "regionModel": "One FTS per managed element: a Database Technology (one per technology), a Database Environment (one per environment), the Stored Instance of a Data Asset (one per asset) and the Business Continuity Plan of an environment run concurrently and are coupled by cross-region constraints, facts and events, never a single subject.",
             "source": SRC_DECK + "; " + SRC_HOWARD + "; " + SRC_PROTOCOL,
             "note": "Four state regions, each its own FTS over one managed element of the Knowledge Area: a Database Technology, a Database Environment, the Stored Instance of a Data Asset and the Business Continuity Plan. The KA never becomes a region of the Data Asset; the environment, the stored instance and the plan reach the Global protocol through contributions (materialisation, active and preservation custody, restoration, the custody transfer trigger, destruction and custody closure, the assurance trigger and the monitoring service), and couple to Data Architecture, Data Modelling and Design, Data Security, DG and Metadata.",
    ...
    "qaNotes": [
        ...
        {"severity": "note", "rule": "deviation", "element": "CON-DSO-02", "finding": "Howard's gating named the backup for active custody; custody is established before the asset is loaded in every scenario (TR-CP-01 follows TR-EX-01), so active custody is gated on the environment in service under a tested continuity plan and the backup gates preservation and restoration. Put to Howard."},
