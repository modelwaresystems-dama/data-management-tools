#!/usr/bin/env python3
"""
dg_spec.py  -  Data Governance FTS v0.1: five orthogonal regions derived from the DMBOK Data Governance
context diagram (deck pages 18 to 21), aligned to the Global Data Asset Protocol.
...
Subject (decision 20 Sep 2026): the organisation's Data Governance capability for a governed scope.
Region 5 is drafted as the condition of the capability's issue handling for a Data Asset; its natural
subject is an issue case, which is flagged in QA.
...
SRC_HOWARD = "Howard Diesel, 20 Sep 2026: five orthogonal DG regions and the start-from-definition method"
...
CONTEXT = {
    ...
    "processes": [
        ...
        {"id": "4", "name": "Embed Data Governance", "phase": "C,O", "subActivities": [], "note": "Sub-activities are cut off on the slide; to be supplied by Howard."},
    ],
...
CAP = "Data Governance capability of the organisation for a governed scope"
REGIONS = [
    ("REG-DG-STR", "Strategy and Value Realisation", "STR", "Whether an approved data and data management strategy exists, is being executed, and is realising recognised data value.", "STS-STR-01", "Exactly one active state; a revised strategy supersedes, never coexists with, the approved one.", CAP),
    ("REG-DG-OPM", "Operating Model", "OPM", "Whether the federated operating model, ownership and stewardship (custody) arrangements are defined, staffed and operating.", "STS-OPM-01", "Exactly one active state; decision rights exist only from Roles Assigned onward.", CAP),
    ("REG-DG-RDY", "Readiness", "RDY", "Whether the organisation's maturity, culture and change capacity for data governance are known and being raised.", "STS-RDY-01", "Exactly one active state; readiness is a measured condition, never asserted.", CAP),
    ("REG-DG-POL", "Principles, Policies, Procedures and Business Reference Architecture", "POL", "Whether the governing instruments (principles, policies, procedures, standards, glossary, reference architecture) exist, are approved and are in force.", "STS-POL-01", "Exactly one active state; an instrument in force is the only one a Global guard may cite.", CAP),
    ("REG-DG-ISS", "Data Asset Issue Management", "ISS", "Whether a governance issue on the Data Asset is open, being resolved, escalated, or resolved.", "STS-ISS-01", "Exactly one active state per Data Asset; an escalated issue blocks material Global transitions until resolved or accepted.", "Data Asset issue case (subject differs from the capability; see QA)"),
]
STATES = [
...
EVENTS = {
    ...
    "EV-ISS-01": ("Issue logged", "Request"), "EV-ISS-02": ("Resolution assigned", "Decision outcome"), "EV-ISS-03": ("Escalation", "Decision outcome"), "EV-ISS-04": ("Resolution or risk acceptance", "Decision outcome"), "EV-ISS-05": ("Issue closure", "Evidence trigger"), "EV-ISS-06": ("Issue reopened", "Monitoring trigger"),
}
DR = {
...
TRANS = [
    ("TR-STR-01", "Formulate Strategy", "STS-STR-01", "STS-STR-02", "EV-STR-01", "A sponsor and scope exist; business and IT strategies and goals are available as inputs.", None, ["SVC-DG-01"], ["ACT-DG-1.1", "ACT-DG-1.3"]),
    ...
    ("TR-STR-04", "Evidence Value", "STS-STR-04", "STS-STR-05", "EV-STR-04", "Data asset valuation is sponsored and recognised value is reported.", None, ["SVC-DG-02"], ["ACT-DG-3.4"]),
    ("TR-STR-05", "Revise Strategy after Execution", "STS-STR-04", "STS-STR-06", "EV-STR-05", "A material change in business or regulatory direction is recorded.", None, ["SVC-DG-01"], ["ACT-DG-1.3"]),
    ("TR-STR-06", "Revise Strategy after Value", "STS-STR-05", "STS-STR-06", "EV-STR-05", "A material change in business or regulatory direction is recorded.", None, ["SVC-DG-01"], ["ACT-DG-1.3"]),
    ...
    ("TR-STR-08", "Withdraw Strategy", "STS-STR-06", "STS-STR-01", "EV-STR-07", "The strategy is withdrawn without replacement and the withdrawal is recorded.", "DR-DG-01", [], []),
    ("TR-OPM-01", "Design Operating Model", "STS-OPM-01", "STS-OPM-02", "EV-OPM-01", "A design mandate exists; organisational touchpoints are being identified.", None, ["SVC-DG-03"], ["ACT-DG-2.1", "ACT-DG-1.4"]),
    ...
    ("TR-OPM-05", "Restructure Operating Model", "STS-OPM-05", "STS-OPM-06", "EV-OPM-05", "A restructuring trigger is recorded; roles in force are retained meanwhile.", None, ["SVC-DG-03"], ["ACT-DG-2.1"]),
    ...
    ("TR-OPM-07", "Record Role Vacancy", "STS-OPM-05", "STS-OPM-03", "EV-OPM-07", "An owner, steward or body role required for the scope is vacant.", None, ["SVC-DG-04"], []),
    ("TR-RDY-01", "Initiate Readiness Assessment", "STS-RDY-01", "STS-RDY-02", "EV-RDY-01", "Assessment scope, assessors and criteria are defined.", None, ["SVC-DG-05"], ["ACT-DG-1.2"]),
    ("TR-RDY-02", "Establish Readiness Baseline", "STS-RDY-02", "STS-RDY-03", "EV-RDY-02", "Maturity level, culture findings and gaps are accepted by the sponsor.", None, ["SVC-DG-05"], ["ACT-DG-1.2"]),
    ...
    ("TR-RDY-04", "Confirm Target Readiness", "STS-RDY-04", "STS-RDY-05", "EV-RDY-04", "Reassessment shows the target maturity and culture indicators are reached.", None, ["SVC-DG-05"], ["ACT-DG-1.2"]),
    ("TR-RDY-05", "Reassess Readiness", "STS-RDY-05", "STS-RDY-02", "EV-RDY-05", "The reassessment interval has elapsed or a lapse is detected.", None, ["SVC-DG-05"], ["ACT-DG-1.2"]),
    ("TR-RDY-06", "Reassess during Change", "STS-RDY-04", "STS-RDY-02", "EV-RDY-05", "A scheduled interim reassessment is due.", None, ["SVC-DG-05"], ["ACT-DG-1.2"]),
    ("TR-POL-01", "Develop Instruments", "STS-POL-01", "STS-POL-02", "EV-POL-01", "Regulatory requirements and organisation policies are assessed as inputs.", None, ["SVC-DG-07"], ["ACT-DG-2.2", "ACT-DG-2.6", "ACT-DG-3.1", "ACT-DG-3.2", "ACT-DG-3.3"]),
    ...
    ("TR-POL-04", "Review Instruments", "STS-POL-04", "STS-POL-05", "EV-POL-04", "A regulatory, strategic or compliance trigger is recorded.", None, ["SVC-DG-07"], ["ACT-DG-2.6"]),
    ...
    ("TR-POL-06", "Retire Instruments", "STS-POL-05", "STS-POL-01", "EV-POL-06", "All instruments for the scope are retired without replacement.", "DR-DG-04", [], []),
    ("TR-ISS-01", "Log Issue", "STS-ISS-01", "STS-ISS-02", "EV-ISS-01", "An issue against the Data Asset is logged with severity and owner.", None, ["SVC-DG-09"], ["ACT-DG-2.5"]),
    ("TR-ISS-02", "Assign Resolution", "STS-ISS-02", "STS-ISS-03", "EV-ISS-02", "A steward is assigned under the operating model and a plan exists.", None, ["SVC-DG-09"], ["ACT-DG-2.5"]),
    ...
    ("TR-ISS-05", "Resolve Issue", "STS-ISS-03", "STS-ISS-05", "EV-ISS-04", "Resolution evidence is accepted by the issue owner.", None, ["SVC-DG-09"], ["ACT-DG-2.5"]),
    ...
    ("TR-ISS-07", "Close Issue", "STS-ISS-05", "STS-ISS-01", "EV-ISS-05", "Closure evidence is retained.", None, ["SVC-DG-09"], ["ACT-DG-2.5"]),
    ("TR-ISS-08", "Reopen Issue", "STS-ISS-05", "STS-ISS-02", "EV-ISS-06", "The resolution failed or recurred.", None, ["SVC-DG-09"], ["ACT-DG-2.5"]),
]
SERVICES = [
    ...
    ("SVC-DG-09", "Governance", "Data Asset Issue Management", "Issue logged against a Data Asset.", "Issue decision; resolution or risk acceptance."),
]
ACTS = [
    ...
    ("ACT-DG-4", "Embed Data Governance", "4", "Transition-causing", ["REG-DG-OPM", "REG-DG-POL"], ["TR-OPM-04", "TR-POL-03"], ["SVC-DG-08"]),
]
ARTEFACTS = [
...
CONTRIB = [
    ("CON-DG-01", "TR-EX-01", "guard", {"OPM": ["STS-OPM-04", "STS-OPM-05"]}, "An accountable Data Owner and Steward can be assigned: the DG Operating Model is at Roles Assigned or in force.", "DG_roles_assigned", "Required", "Register Asset needs an owner on record (GA-001)."),
...
EXC = [("EXC-DG-01", "Interim Governance Exception", "TR-EX-01", "Registration of an urgent Data Asset before the operating model reaches Roles Assigned.", "DR-DG-03", "Interim owner named, expiry set, compensating oversight by a governance body, evidence retained; never waives CON-DG-04.", "Draft / Approved / Expired / Closed")]

SPEC = {
    "meta": {"modelId": "KA-DG", "name": "Data Governance FTS", "knowledgeArea": "Data Governance", "version": "0.1", "subjectType": CAP, "source": SRC_DECK + "; " + SRC_HOWARD + "; " + SRC_PROTOCOL,
             "note": "Five orthogonal regions of the Data Governance capability, each a state machine with its own initial state, derived from the DMBOK context diagram. The KA never becomes a region of the Data Asset; it reaches the Global protocol through contributions (guards, decision rights, services, events) listed on the Contributions sheet and federated onto the Global transitions in the viewer.",
    ...
    "sources": [
        {"id": "SRC-DG-001", "source": SRC_DECK, "type": "Primary (image pages, captured)", "location": "Chat upload; OneDrive Data Lifecycle folder", "use": "Definition, goals, drivers, inputs, processes and sub-activities, deliverables, role players, techniques, tools, metrics", "limitations": "Process 4 Embed Data Governance sub-activities are cut off on the slide."},
        {"id": "SRC-DG-002", "source": SRC_HOWARD, "type": "Design direction", "location": "Chat", "use": "Region set, subject, start-from-definition method", "limitations": "Region 5 subject differs from the capability."},
    ...
    "qaNotes": [
        {"severity": "note", "rule": "capture", "element": "process:4", "finding": "Embed Data Governance (C,O) sub-activities are cut off on the slide; ACT-DG-4 is drafted as one activity until Howard supplies them."},
        {"severity": "note", "rule": "GA-005", "element": "KA-DG", "finding": "Whole-capability regions STR, OPM, RDY and POL change rarely relative to a Data Asset; they gate Global transitions as preconditions (contributions), which is the loose coupling GA-009 asks for."},
