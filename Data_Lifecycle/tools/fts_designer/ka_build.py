...
def build(spec, overrides=None):
    ...
    if m["artefacts"] or m["evidence"]:
        ...
        m["kaCouplings"].append(row({"id": f"KAC-{ka}-{n0+1:02d}", "targetModel": "KA-MM", "targetTransition": "TR-AST-02", "event": "", "expression": "MM_asset_described",
            ...
            "note": "Generated by ka_build.py from Howard's rule (22 Sep 2026): all deliverables and evidence are metadata. Forward coupling: the MM transition cites this KA's artefacts and evidence." + self_note}, "derived:metadata-assets"))
        m["kaCouplings"].append(row({"id": f"KAC-{ka}-{n0+2:02d}", "targetModel": "KA-DQ", "targetTransition": "TR-PDCA-02", "event": "", "expression": "DQ_conforming",
            ...
            "note": "Generated by ka_build.py from Howard's rule (22 Sep 2026). Reverse coupling: this KA's outputs cite the DQ conformance fact." + self_note}, "derived:metadata-assets"))
