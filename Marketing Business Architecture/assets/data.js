...
window.PACK_CONFIG.version = "v1.87.23";
window.PACK_CONFIG.built   = "2026-08-19 20:45 SAST";
window.PACK_CONFIG.changelog = [
  { v:"v1.87.23", date:"2026-08-19 20:45 SAST",
    note:"Fixed AGGPSA AI use-cases contributing to only one business outcome when they contribute to several. The use-case→outcome mapping was one-to-one — the Data & Analytics use-case card showed a single '→ outcome BO05' — even though the model's own Enterprise Value Map ties each use-case's value theme to several outcomes (e.g. VT1 'Systemic ecosystem change' → BO04, BO05, BO08). Made the relationship many-to-many on Howard's chosen basis (union: keep each use-case's current outcome and add the rest of its value theme's outcomes): expanded 163 · UseCase_ValueAssessment with an OutcomeIDs list and refreshed each rationale, and grew 53 · UseCase_BusinessOutcome_Map from 18 to 43 links. The use-case card now lists every contributing outcome (U05 → BO04, BO05, BO08) instead of one. Distribution: 8 use-cases now map to 3 outcomes, 3 to 2, 2 to 4, and 5 stay single where their theme genuinely names one outcome; the three draft use-cases (U19–U21) have no value theme yet and were left untouched. Model data change (workbook + pack bundle) plus the display fix; AGGPSA stays at 0 structural / 0 not-connected gaps and the analytics matrix still plots all 21 use-cases. Nedbank and Modelware unaffected." },
  ...
  { v:"v1.87.20", date:"2026-08-19 18:55 SAST",
    note:"Corrected a mis-classification Howard caught: several decisions typed AI Use-Case were actually human or rule-based, and were nonsensically wired to U01 'Lead prioritisation assistant' (e.g. Moodle Extension Approval, Absence Handling). Re-typed them on Howard's approved reading — Moodle Extension Approval and the other human-managed calls (Channel Mix, Coupon Replacement, Absence Handling, Advisory Scope Approval, Maturity Investment, Advisory Delivery Quality Sign-off) to Decision Support; the deterministic ones (Strategic-Fit / Permanent Filter, Individual-vs-Corporate, Customer Terms, Post-Delivery Billing Eligibility) to Automation; kept only D006 Lead Qualification on the lead-prioritisation AI. Each re-typed decision now points to a correctly-typed mechanism element governed by its control and data product; where a genuine AI use-case legitimately informs a human decision it was kept as the informer (Maturity Investment keeps 'Advisory opportunity detector', Advisory sign-off keeps 'Advisory deliverable QA assistant') rather than deleted. Authored two missing controls (CTL-BR-32/33) for the decisions that had none. Decision mix is now 45 Automation / 29 AI Use-Case / 9 Decision Support; the mechanism tier is 45 Automation / 26 AI Use-Case / 6 Decision Support, none orphaned, every mechanism→control link resolving to a real control. Net effect: Modelware's Coverage (indirect) list went from 8 to 0 — the model is now completely clean at 0 structural, 0 not-connected and 0 indirect, matching Nedbank and AGGPSA. No AI use-case is claimed where the work is really automation or a human call." },
  { v:"v1.87.19", date:"2026-08-19 18:15 SAST",
    note:"Acted on Howard's principle that an automated decision REQUIRES a data product (the ruleset + the data + the recorded outcome), so a decision reaching data only 'via a related branch' is a real gap, not an informational note. Completed the direct thread Decision → Automation/AI mechanism → Control → Data Product → Information Concept → Subject Area for every decision, using the actual data products each decision already consumes (from its process step) rather than inventing any: added the control→data-product links for the 21 automated decisions whose governing control did not yet reach one, and authored six real business-rule controls (CTL-BR-26..31) for D003/D004/D005/D008/D010/D012, whose 'control' had wrongly been a policy id (POL-PD-*) that is not a Control node — re-pointing each decision's Automation element to the new control and linking it to the data product it uses (Consent & purpose→DP02, Outreach eligibility→DP02, etc.). Also linked eight secondary rule/advisory controls to the data their decisions use, and attached the previously loose 'Advisory delivery quality & evidence' control (CTL-ADV-03) to the advisory sign-off decision it governs. Effect: the Coverage (indirect) list for Modelware fell from 265 to 8 — every decision now reads with a clean vertical thread all the way down to Subject Area. The remaining 8 are a single honest category: secondary business-rule controls that are not themselves governed by an AI use-case above them (forcing those to zero would mean asserting AI governance that does not exist). Structural and not-connected gaps stay zero for all three tenants; no mechanism element is orphaned; every mechanism→control link resolves to a real control. Nedbank and AGGPSA unchanged." },
  { v:"v1.87.18", date:"2026-08-19 17:30 SAST",
    note:"Typed the mechanism tier and kept it fully populated, per Howard's direction: the 'AI Use-Case' tier now carries a Type on every element (AI Use-Case / Automation / Decision Support) and every decision keeps a direct, typed down-link into it — nothing is hidden. Rather than leaving the 41 deterministic (Automation-typed) decisions with no down-link — which had made them show on the Coverage (indirect) sheet as 'AI Use-Case reachable only indirectly', looking like unresolved AI gaps — each now points to a first-class Automation element named for its real mechanism (e.g. 'Business rules engine (consent/purpose)', 'MDM / entity-resolution service', 'Analytics job (attribution model)'), and the one decision-support decision points to a Decision Support element. Each new element is governed by the same control as its decision, so the thread mechanism→control→data is complete. The mechanism tier is now 26 AI Use-Case + 41 Automation + 1 Decision Support = 68 typed elements; no element is orphaned. Effect on the informational Coverage (indirect) sheet: the 122 'no direct AI Use-Case' observations on decisions, processes and controls are gone (they now have a real, typed mechanism), dropping indirect notes from 265 to 220. The remaining 220 are genuine deep-tier reachability facts — a Decision or Control reaches Data Product / Information Concept / Subject Area through its process/control branch rather than owning one directly — and are not gaps. Structural and not-connected gaps remain zero for all three tenants; Nedbank and AGGPSA unchanged." },
  { v:"v1.87.17", date:"2026-08-19 16:45 SAST",
    note:"Applied the Modelware digital-decisioning recommendations to the model and gave every decision a mechanism Type. Each of the 83 decisions now carries a Type of Automation, AI Use-Case or Decision Support (41 / 40 / 2), matching the assessment that deterministic decisions need a business-rules engine, not a model. Acting on that: removed the AI use-case links from the 25 Automation-type decisions that had been over-attributed to a use-case (chiefly the 'Lead prioritisation assistant' blanket-linked across the sales/marketing decisions) so deterministic decisions no longer claim an AI model they do not use; linked the 5 quick-win decisions that already had a fitting use-case built but unlinked (D053→Finance exception detector, D058→Advisory opportunity detector, D059→Partner performance monitor, D078→Community engagement recommender, D080→Content QA assistant); and authored 16 genuinely new AI use-cases (U11–U26) for the decisions where prediction/scoring/optimisation adds real value — e.g. Course demand & margin forecaster, Assessment scoring & borderline flagging, Marketing mix & spend optimiser, KPI anomaly detection & root-cause, Operational anomaly detection & triage, Supplier document verification — each mapped to its decision and to its governing control. All three tenants remain at zero structural and zero not-connected gaps; no AI use-case is orphaned. Nedbank and AGGPSA models unchanged." },
  { v:"v1.87.16", date:"2026-08-19 16:00 SAST",
    note:"Dropped the operational-process exemption entirely and made it real: every business process must have a Decision (Explorer v1.9.11). The earlier ProcessType=Operational exemption \u2014 which had been applied by a hardcoded list, not by the client \u2014 is removed from the gap rules. To satisfy the principle that a business process always makes a decision, authored a genuine decision for each of the 20 Modelware processes that previously had none (D064\u2013D083: e.g. Course start go/no-go, Trainer certification pass/fail, Travel booking approval, Supplier compliance acceptance, Advisory delivery quality sign-off), linked each Process\u2192Decision, and gave each a governing control under the Operational Governance Policy (CTL-OP-18..37) wired to the data products the process uses. Result: all 68 Modelware processes now have a decision, nothing is exempted, and structural/not-connected gaps are zero because the decisions are real \u2014 not because anything is hidden. Nedbank and AGGPSA unaffected." },
  { v:"v1.87.15", date:"2026-08-19 15:20 SAST",
    note:"Made the Explorer honest and navigable about exempt elements (Explorer v1.9.10). Some processes legitimately have no Decision because they are classified ProcessType=Operational (execution/reporting/records \u2014 e.g. Send Pre-Course Requirements, Daily Operational Reporting, Corporate Travel Arrangements), and the gap rules exempt operational processes from the Decision requirement. Two problems are fixed: (1) the Tree explorer now shows an \u2018Operational \u00b7 no Decision\u2019 tag on exempt elements and, crucially, no longer dead-ends them \u2014 a process whose next spine tier is empty now drills straight through to its nearest populated tier (its data products), so the \u2018+\u2019 appears and you can reach the data footprint; (2) the Lineage-trace report now lists any exempt elements on the path and states they are exempt by design (not an automatic gap), so \u2018no gaps\u2019 is explained rather than mysterious. This is display/transparency only \u2014 no model or data changes. Note: whether a given process SHOULD be operational is a modelling judgement; reclassifying one to Core makes its missing Decision a real gap." },
  { v:"v1.87.14", date:"2026-08-19 14:40 SAST",
    note:"Refined the sibling ordering so every tier reads in definition order, not alphabetically (Explorer v1.9.9). Elements without an explicit Sequence (e.g. processes) are now ordered by their identifier in natural, number-aware order rather than by name \u2014 so under a capability the processes read P55 Partner Selection & Due Diligence \u2192 P57 Partner QA & Quarterly Review \u2192 P68 Supplier & Compliance Documentation instead of the alphabetical Partner QA / Partner Selection / Supplier. Priority is: explicit lifecycle Sequence first (e.g. Value Stages), then natural identifier order (P2 before P10, VS09-2 before VS09-10), then name as a last tiebreak \u2014 applied consistently across the lineage graph, Tree explorer and trace panels. No model or data changes." },
  { v:"v1.87.13", date:"2026-08-19 14:15 SAST",
    note:"Elements within a tier now display in their real lifecycle order instead of alphabetically (Explorer v1.9.8). The model carries an explicit Sequence on staged elements (e.g. Value Stages), so a value stream\u2019s stages now read in the order the work actually flows \u2014 for Manage Partner Channel that is Select Partner \u2192 Enable Partner \u2192 Run Local Cohort \u2192 Assure Quality \u2192 Review Performance, rather than Assure/Enable/Review/Run/Select. Sibling ordering (Sequence when present, otherwise a natural, number-aware name/ID sort) is applied consistently across the lineage graph, the Tree explorer and the up/down trace panels. No model or data changes." },
  { v:"v1.87.12", date:"2026-08-19 13:45 SAST",
    note:"Strengthened Modelware\u2019s operational governance. Modelware had zero structural and zero not-connected gaps already; the improvement targets the informational \u2018reachable via a related branch\u2019 notes. Authored 17 real business-rule controls (CTL-OP-01..17) under a new \u2018Operational Governance Policy\u2019 \u2014 one per operational decision that previously had no governing control (course & date inclusion, reconciliation, attendance, access remediation, quiz follow-up, competency, certification eligibility, bad-debt, marketing spend, dunning, curriculum gap, version approval, partner standing, reporting priority, capacity, portfolio, performance) \u2014 each governing its decision and process and the governed data products that process uses (26 Control\u2192Data-Product links). Also measured the two value streams that had no KPI (Shape Strategy & Portfolio, Develop AI Learning Products) by the KPIs of their value proposition\u2019s outcomes. Coverage-indirect notes fell from 398 to 278. The remainder is legitimate and was left as honest observations: Modelware runs many operational decisions that are deliberately NOT AI-assisted, so the AI Use-Case tier is genuinely off their direct thread \u2014 forcing it to zero would mean asserting AI use-cases the business does not have. Nedbank and AGGPSA unchanged." },
  { v:"v1.87.11", date:"2026-08-19 13:10 SAST",
    note:"Stopped the Tree explorer from showing the same element several times (Explorer v1.9.7). Because the model is a network, an element like a control is reachable by several routes \u2014 Process\u2192Control directly, Process\u2192Decision\u2192Control, and Process\u2192Decision\u2192AI\u2192Control \u2014 so it was appearing three times under one process. The tree now walks only the clean adjacent-tier spine by default (Stakeholder\u2192Value Proposition\u2192Business Outcome\u2192KPI\u2192Value Stream\u2192Value Stage\u2192Capability\u2192Process\u2192Decision\u2192AI Use-Case\u2192Control\u2192Data Product\u2192Information Concept\u2192Subject Area), exactly like the lineage graph, so each element appears under its one spine parent (a control now sits only under the AI use-case it governs, not also under the process and decision). The tier-skipping relationships are still available on demand via a new \u2018Show cross-tier links\u2019 tick-box on the Tree explorer, mirroring the graph. No model or data changes." },
  { v:"v1.87.10", date:"2026-08-19 12:40 SAST",
    note:"Rebuilt the Tree explorer to expand lazily (Explorer v1.9.6), fixing two problems from the previous build: nodes that appeared more than once in a densely-connected model were collapsed to a dead \u2018\u2197 shown above\u2019 reference and could not be opened, and \u2018Expand all\u2019 on the fully-connected Nedbank model overflowed the page and threw a RangeError. The tree now renders one level at a time and fills each node\u2019s children on click, so every occurrence of an element is independently expandable all the way down its path \u2014 the Tree explorer now mirrors the Lineage-trace path exactly (e.g. Value Stream \u2192 Value Stage \u2192 Capability \u2192 Process \u2026), children stay in spine/tier order, per-branch cycles are marked, and \u2018Expand all\u2019 opens breadth-first up to a bounded budget so it can never overflow. No model or data changes." },
  { v:"v1.87.9", date:"2026-08-19 12:10 SAST",
    note:"Added a Subject Area layer to Modelware — all three tenants now have one. Modelware's 39 information concepts are classified into ten data-estate areas fitting a whole training business: Party & Relationships, Marketing & Demand, Product & Curriculum, Sales & Commercial, Learning Delivery, Certification & Entitlement, Advisory & Maturity, Governance/Compliance & Evidence, AI & Semantic Assets, and Performance & Reference (223 SubjectArea + 222 Concept→SubjectArea, every concept classified, no orphan areas). Structural and not-connected gaps stay at zero. Modelware still shows informational 'reachable via a related branch' notes on the AI Use-Case / Control tiers — these are legitimate (operational decisions and processes that genuinely have no AI use-case on their direct thread) and are not gaps, so they were left as honest observations rather than papered over with fabricated links. Nedbank and AGGPSA are unchanged." },
  { v:"v1.87.8", date:"2026-08-19 11:40 SAST",
    note:"Added a Subject Area layer to Nedbank (previously only AGGPSA had one) and fixed two Tree-explorer issues (Explorer v1.9.5). Nedbank's 18 information concepts are now classified into a seven-area marketing-bank data estate — Party & Customer, Product & Offer, Marketing & Campaign, Interaction & Experience, Financial & Value, Risk & Compliance, and Analytics & Data Governance (223 SubjectArea + 222 Concept→SubjectArea) — so the spine runs all the way to Subject Area and Nedbank stays fully clean (0/0/0). Modelware still has no Subject Area layer (that's the next candidate if wanted). Tree-explorer fixes: (1) children are now listed in spine/tier order (Decision → AI Use-Case → Control → Data Product → Information Concept → Subject Area) to match the Navigator and lineage graph, instead of alphabetically mixing element types; (2) a global 'expand once' guard stops the tree from re-expanding shared nodes in densely-connected models — that runaway expansion was overflowing the page's HTML string and throwing a RangeError on load for the newly-connected Nedbank model. Repeated nodes now show a '↗ shown above' reference." },
  { v:"v1.87.7", date:"2026-08-19 11:00 SAST",
    note:"Nedbank Marketing is now fully clean on every measure — zero structural, zero not-connected AND zero coverage-indirect (the remaining 990 'reachable via a related branch' notes are gone), matching AGGPSA. Three direct links closed them: (1) KPI → Value Stream — each value stream is now measured by the KPIs of the outcomes of the value proposition it realises (93 links derived from the value chain), so the value side and the delivery side share one vertical thread; (2) a clean AI Use-Case → Control → Data Product thread — each of the 18 AI use-cases is now governed by the existing control(s) that govern the decision(s) it supports (221 map, 32 links) and those controls govern the data products the use-case consumes (220 extended, 106 links), reusing Nedbank's own CTL-NB controls rather than inventing new ones; (3) eight Information Concepts were added so the foundational data products (Customer 360 Profile, Product Holdings, Behavioural Event Stream, Transaction Ledger, Risk & Affordability Signals, Feature Store, Journey Analytics, Data Quality & Lineage) each materialise a concept directly. Modelware and AGGPSA are unchanged." },
  { v:"v1.87.6", date:"2026-08-19 10:05 SAST",
    note:"Resolved every structural gap in the Nedbank Marketing architecture — it now returns zero structural and zero not-connected gaps (down from 55 and 666). The root causes were an under-mapped value chain: the seven KPI outcome-themes are now mapped one-to-one onto the seven business outcomes (Acquisition→Increase quality acquisition, Relationship growth→Increase relationship depth, Engagement→Improve customer experience, Wealth→Increase customer lifetime value, Trade finance→Increase trade revenue, Brand & trust→Strengthen brand & trust, Marketing performance→Improve marketing performance & efficiency), which reconnects all 34 orphan KPIs and gives the two outcomes that had no KPI their measures. The seven value propositions that had no business outcome were mapped to the outcome they drive (e.g. Governed Marketing Performance→Improve marketing performance & efficiency, Consistent Omnichannel Experience & Right-First-Time Fulfilment→Improve customer experience, Controlled Responsible Growth & Transparent Responsible Communication→Strengthen brand & trust), which also re-parents the three orphan outcomes. The orphan 'Human Review' decision (DX8) is now invoked by the Personalisation Operations and Design-offer processes, and the eight foundational data products (Customer 360, Product Holdings, Behavioural Event Stream, Transaction Ledger, Risk & Affordability Signals, Feature Store, Journey Analytics, Data Quality & Lineage) are now governed by their data controls (consent/privacy, offer-eligibility, next-best-action and measurement). Remaining coverage notes are the informational 'reachable via a related branch' kind, not gaps. Modelware and AGGPSA are unchanged." },
  { v:"v1.87.5", date:"2026-08-19 09:10 SAST",
    note:"Gave Modelware the same clean AI Use-Case → Control → Data Product → Information Concept thread that AGGPSA has. Authored a new 'AI & Model Governance Policy' with one responsible-use control per AI use-case (CTL-AI-01..10), each governing its use-case and the specific data product(s) that use-case consumes; wired AI Use-Case → Control (new 221 map, 10 links) and extended Control → Data Product (220, +15 links). The two use-cases that previously consumed no data product were given the domain-fit product they clearly serve (Content QA → Content Asset & Version Register; Finance exception detector → Quote, Invoice & Payment Ledger). Every Modelware AI use-case now traces straight down to its controls, data products and information concepts on the direct thread — the 'reachable only indirectly' notes for the Control / Data Product / Information Concept tiers are gone (Modelware's indirect-coverage observations dropped from 638 to 356, and remain purely informational; structural and not-connected stay at zero). Nedbank and AGGPSA are unchanged." },
  { v:"v1.87.4", date:"2026-08-18 16:40 SAST",
    note:"Fixed a confusing mismatch between the Lineage & Gap Explorer's on-screen report and its Excel export (v1.9.4). The Excel already separated a real 'Gaps' worklist (structural problems + spine tiers that are not connected at all) from a much larger, informational 'Coverage (indirect)' sheet (tiers that ARE reachable, just via a related branch rather than one clean vertical thread). The on-screen report, however, lumped both together and shouted '⚠ 3 gaps on this lineage' even when the Excel's Gaps sheet was empty. The report now uses the same split: it counts only structural + not-connected as actual gaps (so an element with no real problems shows a green '✓ No gaps'), and lists the indirect-reachability items separately as a soft, grey 'ⓘ … not a gap, just no single clean vertical thread' note. The graph's empty-tier markers match — a tier reachable only indirectly now shows a neutral blue 'via related branch' chip instead of an amber 'coverage gap ⚠'. This is purely how gaps are labelled and counted; no model or data changed, and the numbers now agree with the exported workbook." },
  { v:"v1.87.3", date:"2026-08-18 15:58 SAST",
    note:"Added hover tooltips to the Lineage & Gap Explorer graph (v1.9.3): where a node's name is too long to fit and shows with a trailing “…”, hovering the box now pops an instant tooltip with the complete label — element type, full name and ID (and any “missing …” note). The tooltip follows the cursor, flips to stay on-screen near the edges, and disappears as you move off the node. No model or gap changes." },
  { v:"v1.87.2", date:"2026-08-18 15:30 SAST",
    note:"Two big usability changes to the Lineage & Gap Explorer (v1.9.2). First, the graph is much cleaner: it now draws only the tidy adjacent-tier flow (Stakeholder→…→Decision→AI Use-Case→Control→Data Product→Information Concept→Subject Area) and no longer clutters the picture with the tier-skipping 'shortcut' lines — e.g. an Information Concept looping back up to a Capability, Process or Decision. Those relationships still exist in the model and can be shown on demand with a new 'Show cross-tier links' tick-box; they are just hidden by default so the flow reads left-to-right. Second, you can now select MULTIPLE elements as filters instead of one: type them in, or click nodes in the graph to add/remove them, and the diagram narrows to only the elements connected to ALL your selections (an intersection). This lets you pick, say, an AI Use-Case plus a Stakeholder plus a Value Stage and see just the single branch that ties them together, cutting out the wide fan of unrelated upstream elements. Selected elements show as removable chips with a 'clear all'. Also fixed a latent bug where the Ancestors/Descendants direction buttons silently did nothing (they read the wrong attribute), so direction filtering now works. No model or gap changes — all three tenants are exactly as in v1.87.0." },
  { v:"v1.87.1", date:"2026-08-18 14:52 SAST",
    note:"Made the Lineage & Gap Explorer's graph the main event: it now fills the browser window (roughly full height instead of a fixed 70%), so you see far more of the spine without scrolling up and down. Added a second horizontal scrollbar pinned at the TOP of the graph, kept in sync with the bottom one, so you can slide sideways to the Control / Data Product / Information Concept / Subject Area tiers immediately — no need to scroll to the bottom of the page to reach the slider. The gap report above the graph is now capped to a compact scrollable strip so a long gap list can't push the graph off-screen. Also wired the header version badge to the live app version so it stops drifting (Explorer v1.9.1). No model or gap changes — all three tenants are exactly as in v1.87.0." },
  { v:"v1.87.0", date:"2026-08-18 14:20 SAST",
    note:"Re-pointed the Lineage & Gap Explorer's governance flow so it reads left-to-right in the order you asked for: after Decision it now goes AI Use-Case → Control → Data Product → Information Concept → Subject Area, and the lineage graph is bounded to a scrollable panel so the newly-added bottom tier is visible without scrolling the whole page (Explorer v1.9.0). Added a new terminal Subject Area tier for AGGPSA built from the EnVision Data Estate — seven areas (Party Master, Product, Financial, Transactional, Marketing, Contract/Agreement, Reference) with all 30 information concepts classified into them (223 SubjectArea + 222 Concept→SubjectArea maps). Reversing the concept↔data-product edge to honour the new order re-orphaned four Modelware data products and one AGGPSA AI use-case that had only been parented the old way; these were reconnected with genuine relationships — the four Modelware products now sit under the data controls that govern them (Data quality, Auditability, Reconciliation, Stable-person-identity; new 220 map), and the AGGPSA ecosystem-dashboard use-case plus the Launchpad, Train-the-Trainer and participant-onboarding decisions were linked to the AI use-cases that support them. A tenant guard now skips empty tiers so the new Subject Area column is not flagged as a gap for Nedbank/Modelware, which do not use it. AGGPSA remains fully clean (zero structural, zero coverage-not-connected, zero coverage-indirect); Modelware stays zero structural / zero coverage-not-connected; Nedbank's baseline is unchanged." },
  { v:"v1.86.0", date:"2026-08-18 12:35 SAST",
    note:"Resolved every remaining AGGPSA gap — structural, coverage-not-connected AND coverage-indirect are now all zero. The 449 'reachable indirectly' observations were the spine's parallel branches (outcome/KPI vs value stream, and process/decision vs concept/data/AI) reaching each other only sideways; they were closed by adding the genuinely-meaningful direct links: every value stream is now measured by the KPI(s) of its value proposition's outcomes (line of sight to value); each process and decision links to the information concepts it uses (217/218 maps); and each information concept, data product and AI use-case links to the control that governs it (219/220/221 maps), with five new controls (CTL-AG-19..23) authored under a new Data & Measurement Governance Policy to govern the EnVision-added decisions, plus the missing data-product↔AI-use-case links. The Explorer (v1.8.0) reads these new per-tenant maps (they no-op for Nedbank/Modelware, which are unchanged). Fixed a sheet-name-truncation bug where map sheets named beyond Excel's 31-character limit were duplicated and read stale. The AGGPSA pack model bundle and workbook were regenerated together; AGGPSA now returns a fully empty gap export." },
  { v:"v1.85.0", date:"2026-08-18 11:57 SAST",
    note:"Aligned the AGGPSA model to the EnVision V1 strategy, closed all its gaps, and propagated everything into the pack. Added 44 strategy-derived elements — LSSB and ecosystem-health outcomes (with KPIs); the AG Makers 5-phase readiness journey (Technical→Entrepreneurial→Business→Industry→Market) as value stages and processes; Launchpad, Train-the-Trainer, participant onboarding/journey-mapping, Makers-Movement convening and Group-Integrated-Measurement processes; capabilities for Entrepreneur Readiness & Journey Mapping, Educator Enablement / Train-the-Trainer, and Integrated Measurement & Group Reporting (IMF/TOC); the Party master (Participant, Funding Partner, TVET College, Ecosystem/Facilitation Partner, Policy Maker) plus Course, TOC, IMF, Initiative and Contract concepts; data products for the Party/Participant MDM, IMF store, TOC store, Ecosystem Conditions Regional Dashboard, Public Glossary and Course Catalogue; and readiness-gate / IMF-attribution decisions and AI use-cases. Then closed AGGPSA's 26 pre-existing structural gaps (value propositions with no value stream, orphaned outcomes/KPIs/decisions/data products) — which also cleared all 136 coverage gaps including the 10 Control-tier ones, since the orphans had been fragmenting the graph. Added a KPI→Value-Stream map so AGGPSA KPIs are no longer dead-ends. The AGGPSA pack model bundle (aggpsa_model_all.js, 190 sheets) and the FutureState workbook were regenerated from one source so the pack app and the Explorer now match; Nedbank was left untouched. AGGPSA now returns zero structural and zero coverage gaps." },
  { v:"v1.84.0", date:"2026-08-17 18:38 SAST",
    note:"Closed the KPI dead-end. Every KPI linked up to a Business Outcome but down to nothing, so KPIs terminated the spine and 'a KPI that doesn't link to anything' showed on traces. The model never populated the 'Outcome → KPI → Value Stream' step as edges — the KPI sheet's AppliesTo only points back up to its outcome, and only five value stages declared a PrimaryKPI. Added 216 · KPI_ValueStream_Map linking all 14 KPIs to the value stream that delivers/measures each (by theme: Leads generated / Response rate → Generate Qualified Demand; Conversion to sale / Quote-to-win → Convert Client Opportunity; Registration readiness → Onboard Learner; Completion rate / Certification pass rate → Deliver Learning Outcome; Advisory pipeline → Deliver Advisory Outcome; Reconciliation exceptions → Run Finance & Administration; Partner performance → Manage Partner Channel; Content currency → Develop Curriculum & IP; Reporting reliability → Operate Platforms & Data; Evidence currency → Govern Quality, Risk & Compliance; Alumni engagement → Grow Community & Alumni). Explorer v1.7.0 adds the matching KPI → Value Stream link, so KPIs now flow down into delivery and are no longer dead-ends (e.g. 'Advisory pipeline' now spans 9 tiers down and reads fully connected). Side benefit: because outcomes and KPIs now reach delivery on a direct thread, the Explorer's indirect-coverage observations for Modelware dropped from ~1,404 to ~572. Actionable gap worklist remains zero. No other model data changed." },
  { v:"v1.83.0", date:"2026-08-17 17:52 SAST",
    note:"Closed the 18 Control-coverage gaps in the Modelware model by giving the Advisory value chain its missing governance controls, tied to a policy. Added a new policy BP-ADV 'Advisory & Governance Policy' (211 · BusinessPolicy, owner ROLE-DATA) covering how data & AI advisory engagements are scoped, delivered and evidenced. Under it, authored three controls (212 · BusinessPolicyControl): CTL-ADV-01 'Advisory scope & independence approval' governing decision D017 (Advisory Scope Approval) on process P43; CTL-ADV-02 'Maturity assessment evidence & recommendation control' governing D058 (Maturity investment) on P53; and CTL-ADV-03 'Advisory delivery quality & evidence control' applied to P54 (Advisory Delivery & Mentoring). With these, every advisory process and decision now reaches the Control tier, so the whole advisory branch (Stakeholder 'Advisory / consulting clients' → VP 'Practical Data Management Enablement' → 'Deliver Advisory Outcome' stream and its stages → 'Data & AI Advisory & Governance' capability → advisory processes/decisions → Maturity Assessment / Governance Artefact concepts) is fully connected. The Explorer's actionable gap worklist (structural + not-connected coverage) for Modelware is now zero (Controls 25→28). No other model data changed." },
  { v:"v1.82.0", date:"2026-08-17 17:34 SAST",
    note:"Lineage & Gap Explorer v1.6.0 — the Export gaps button now exports COVERAGE gaps, not just structural ones, so what the trace shows and what the file contains are consistent. Previously, with all structural gaps closed, the export reported 'nothing to export' even while a trace clearly showed a coverage gap (e.g. the advisory Stakeholder's 'Control — not connected at all'). The export now has three sheets: 'Gaps' (the actionable worklist = structural gaps + genuinely-disconnected 'Coverage — not connected' gaps, each with a suggested fix), 'Coverage (indirect)' (the reachable-via-a-related-branch observations, kept separate so the worklist stays actionable), and 'Summary' (counts plus totals for each class). For the current Modelware model this yields 18 actionable coverage gaps — all of them the Control tier, because the entire Advisory value chain (Deliver Advisory Outcome → its stages → Data & AI Advisory capability → the advisory processes and decisions) has no governing Control, so nothing in that branch reaches the Control tier. The 1,382 indirect observations (mostly Business Outcome / KPI reachable sideways through a value proposition) are on the second sheet. Presentation only — no model data changed." },
  { v:"v1.81.0", date:"2026-08-17 17:15 SAST",
    note:"Lineage & Gap Explorer v1.5.0 — the Lineage trace now reports COVERAGE gaps, not just structural ones. Any spine tier the selected element does not reach on its direct vertical thread (straight up through parents / straight down through children) is now flagged as a coverage gap in the trace report and shown as an amber, flagged column in the diagram instead of a silent 'not on this path'. Each coverage gap is annotated as either 'reachable only indirectly (via a related branch)' — e.g. a Capability reaches Business Outcome/KPI only sideways through its Value Propositions — or 'not connected at all'. The verdict now reads '✓ No gaps' only when the element is fully connected on the direct thread AND every element on the path has its required parents and children; otherwise it lists structural gaps and coverage gaps separately with counts. Example: 'Learning-Coach AI & Product Development' now shows 3 coverage gaps (Business Outcome, KPI, Control), 'Determine Certificate Level' shows 2, while a Stakeholder that reaches all twelve tiers below it still shows no gaps. The Export-gaps dialog message now clarifies that the export covers structural gaps and that per-element coverage gaps live in the trace report. Presentation only — no model data changed." },
  { v:"v1.80.0", date:"2026-08-17 16:52 SAST",
    note:"Lineage & Gap Explorer v1.4.0 — completed the spine so a Process now shows the data it actually touches. Previously Information Concepts and Data Products were only linked to Capabilities, so tracing a process (e.g. 'Determine Certificate Level') showed Information Concept and Data Product as 'not on this path' even though the process demonstrably reads/writes them — which looked like a false 'no gaps'. The Explorer now bridges Process → ProcessStep → Information Concept and Process → ProcessStep → Data Product using the step maps already in the workbook (133 Process_ProcessStep, 135 ProcessStep_InformationConcept, 136 ProcessStep_DataProduct), so a process's real data footprint appears on its trace. 'Determine Certificate Level' now connects to the Assessment and Certificate concepts, the Learner-Progress and Certification-Entitlement data products, and its governing Control — spanning five tiers up and five down. Business Outcome and KPI remain correctly off-path (a process rolls up to them through the value-stream chain, not by a direct link). The bridge is generic and will light up for Nedbank and AGGPSA too once their step maps are present. No model data changed." },
  { v:"v1.79.0", date:"2026-08-17 16:45 SAST",
    note:"Lineage & Gap Explorer upgraded to v1.3.0 so the Lineage trace view itself reports gaps and shows the full spine. Previously a trace only drew the tiers an element actually touched, so empty columns (e.g. Business Outcome, KPI, Process, Decision, Control) silently disappeared and it looked like the spine was broken. The trace now always renders the complete 13-tier spine skeleton and marks any tier the element does not reach as 'not on this path' (a parallel branch, not a gap). A new report banner above the trace states, for the selected element: how many tiers its connected lineage spans up and down and how many elements are on the path; a clear verdict — '✓ No gaps on this lineage' when every element on the trace has its required parents and children, or '⚠ N gaps' with each offending element and what it is missing (missing required child, or orphan with no parent); and the list of off-path tiers with the explanation that they sit on parallel branches. This directly answers the case where tracing a Data Product up through Information Concept → Capability → Value Stage → Value Stream → Value Proposition → Stakeholder legitimately does not pass through Business Outcome, KPI, Process, Decision or Control. Presentation only — no model data changed since v1.78.0." },
  { v:"v1.78.0", date:"2026-08-17 16:33 SAST",
    note:"Remediated all 52 architecture gaps flagged by the Lineage & Gap Explorer for the Modelware model, at the model source (apply_modelware_gaps.py in the build chain) so the fixes flow into both the workbook and the pack. Structural links added: VP10 (Credible Certification Preparation) now realises Value Stream VS05 (Deliver Learning Outcome); Business Outcome BO03 (Improve learner readiness) is now driven by VP06; the seven capabilities with no process (Brand & Market Positioning, CRM, Financial Management, Lead & Pipeline, Learning-Coach AI, Operations & Admin, Outreach & Automation) are mapped onto their genuine host processes; the three unparented capabilities (Brand, Career Coaching, People & Talent) are placed on value stages; and the two orphan Data Products (Partner Channel Performance, Community & Alumni Engagement) now hang off two new Information Concepts (Partner Channel under C21, Community & Alumni Member under C10). For the 38 processes with no decision, 18 that make a genuine choice received authored decisions D046–D063 (each with inputs, rule, HITL, outcomes, Process_Decision_Map, DecisionOutcomeHandler and a BPMN gateway on the concluding step); the remaining 20 purely operational/monitoring processes were classified as ProcessType=Operational and are exempted from the decision requirement (Explorer v1.2.0 honours the exemption). Decisions grew 45→63, Information Concepts 37→39. Gap export now returns zero missing-required and zero orphan gaps for Modelware." },
  { v:"v1.77.0", date:"2026-08-17 15:47 SAST",
    note:"Extended the Lineage & Gap Explorer (now v1.1.0) to include the Data Product and Control tiers in the spine, so it no longer stops at the AI Use-Case. The full trace is now Stakeholder → Value Proposition → Outcome → KPI → Value Stream → Stage → Capability → Process → Decision → Information Concept → Data Product → AI Use-Case → Control. Data Products are linked from the Information Concept that names them as its primary data product (126 · PrimaryDataProductID) and flow down to the AI Use-Cases that consume them (189 · DataProduct_AIUseCase_Map); Controls are linked up to the Decision they govern and the Processes they apply to (212 · BusinessPolicyControl). The tiers colour-code as DP (green) and CTL (amber) with their own badges. Tenants without a control catalogue (Nedbank, AGGPSA have no sheet 212) simply show no Control nodes — the parser is graceful. Verified on all three FutureState workbooks: Modelware resolves 15 Data Products and 25 Controls; Nedbank and AGGPSA each resolve 17 Data Products and 0 Controls. No model data changed." },
  { v:"v1.76.0", date:"2026-08-17 17:30 SAST",
    note:"Added the standalone Business Architecture Lineage & Gap Explorer (v1.0.0) and linked it into the pack. A new 'Lineage & Gap Explorer ↗' launch link in the sidebar opens the tool in a new tab (lineage_explorer.html, shipped with the pack). The Explorer is a single self-contained HTML file (SheetJS inlined, fully offline) that reads any of the three FutureState workbooks — auto-detecting the tenant — and traces the full spine (Stakeholder → Value Proposition → Outcome → KPI → Value Stream → Stage → Capability → Process → Decision → Information Concept → AI Use-Case) top-to-bottom and bottom-to-top. It has three views: a Lineage trace (layered graph plus ancestor/descendant trees, with an element picker and direction toggle), a Tree explorer that highlights elements missing a required child or parent (with an 'only gaps' focus), and a Pivot that counts children by type with filters and CSV export. It also exports a Gaps worklist — every element missing a required child or a parent — as an .xlsx (Gaps + Summary sheets, with Status / Owner / Notes columns and autofilter) to drive remediation. No model data changed." },
  { v:"v1.75.0", date:"2026-08-17 16:20 SAST",
    note:"Made the Nedbank and AGGPSA process-landscape lifecycle tabs interactive. Because their lifecycle stages cross-cut the value-chain columns (a stage is served by processes in several domains), a tab now HIGHLIGHTS the processes that serve the selected stage and dims the rest, rather than filtering columns. Nedbank's mapping is DERIVED from the journey model — each process's stage is taken from the customer-journey stage it operationalises (Acquire / Onboard / Serve / Grow / Retain / Advocate) — so its tabs are live with no authoring. AGGPSA's four streams (Grant Lifecycle, Diagnostic Study, Programme Delivery, Impact & Policy) don't map to the journey model, so they are authored per process in the AGGPSA config. Every landscape now also gets an 'All' tab to clear the filter, steering and enabling bands stay visible when a stage is selected, and Modelware keeps its by-phase column filter. Presentation only — no model data changed." },
  { v:"v1.74.0", date:"2026-08-17 15:30 SAST",
    note:"Fixed the Business Process Landscape for Modelware. It had been inheriting Nedbank's marketing landscape (Acquire/Onboard lifecycle, Insight & Targeting / Design & Origination domains) because Modelware carried no landscape config, and the lifecycle tabs did nothing (no click handler) — so it looked stuck. The landscape is now DERIVED from Modelware's own model: a Steering & Governance band (Strategy & Portfolio Planning, Business Performance Review, Data Capability / Governance Assessment, Business Rules & Use Case Management, Supplier & Compliance Documentation), a Core band of the seven value-chain phases the process catalogue actually defines (Planning & Setup → Marketing → Sales → Administration → Training → Completion → Follow-up & Operations) holding all 48 operational processes, and an Enabling & Support band of the remaining support processes — every cell links through to its process, AI-assisted processes highlighted. The phase tabs now work: 'All' shows the whole value chain and each phase tab focuses the Core band on that phase. Nedbank and AGGPSA keep their curated baked landscape; their lifecycle tabs no longer blank the band when clicked (a tab that doesn't map to a column is a safe no-op). Presentation only — no model data changed." },
